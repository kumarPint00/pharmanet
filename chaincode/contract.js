...
class PharmanetContract extends Contract {
  ...
  async registerCompany(
    ...
    helper.validateUserRole(
      ctx,
      [organisationRole.toLowerCase()],
      "User is not authorised to register a Company for role : " +
        organisationRole
    );

    // Create a new composite key for the new company account
    let companyCompositeKey = ctx.stub.createCompositeKey(companyNameSpace, [
      companyCRN,
      companyName,
    ]);
  ...
  async addDrug(ctx, drugName, serialNo, mfgDate, expDate, companyCRN) {
    ...
    helper.validateUserRole(
      ctx,
      ["manufacturer"],
      "Only Manufacturer can add a new Drug"
    );
    ...
    if (companyObject.organisationRole !== "Manufacturer") {
      throw new Error(
        "input companyCRN" +
          companyObject.companyCRN +
          "(" +
          companyObject.companyName +
          ")" +
          " does not belong to a Manufacturer"
      );
    }

    // Create a new composite key for the new drug to be added
    let drugCompositeKey = ctx.stub.createCompositeKey(drugNameSpace, [
      drugName,
      serialNo,
    ]);
    ...
    if (!(!existingDrugBuffer || existingDrugBuffer.toString().length <= 0)) {
      throw new Error(
        "Invalid drug key: " +
          ...
          "+" +
          serialNo +
          ". A drug with this ID already exists."
      );
    } else {
  ...
  async createPO(ctx, buyerCRN, sellerCRN, drugName, quantity) {
    ...
    helper.validateUserRole(
      ...
      "Purchase order can be created by Distributor or Retailer only"
    );
    ...
    helper.validateUserRole(
      ctx,
      [buyerCompanyObject.organisationRole.toLowerCase()],
      "User does not have privilages to create the PO as a " +
        buyerCompanyObject.organisationRole
    );
    ...
    if (
      !(
        (buyerCompanyObject.hierarchyKey === "2" &&
          sellerCompanyObject.hierarchyKey === "1") ||
        (buyerCompanyObject.hierarchyKey === "3" &&
          sellerCompanyObject.hierarchyKey === "2")
      )
    ) {
      throw new Error(
        "Transfer of drug does not follow the hierarchy : buyerHierarchy = " +
          buyerCompanyObject.hierarchyKey +
          "& seller Hierarchy = " +
          sellerCompanyObject.hierarchyKey
      );
    }

    // Create a new composite key for the new purchase order
    let poCompositeKey = ctx.stub.createCompositeKey(poNameSpace, [
      buyerCRN,
      drugName,
    ]);
    ...
    if (!(!existingPOBuffer || existingPOBuffer.toString().length <= 0)) {
      throw new Error(
        "PO already exists. Please try to udpate teh PO: " +
          buyerCRN +
          "+" +
          drugName +
          "."
  ...
  async createShipment(ctx, buyerCRN, drugName, listOfAssets, transporterCRN) {
    ...
    let shipmentCompositeKey = ctx.stub.createCompositeKey(shipmentNameSpace, [
      buyerCRN,
      drugName,
    ]);
    ...
    if (
      !(
        !existingShipmentBuffer || existingShipmentBuffer.toString().length <= 0
      )
    ) {
    ...
    if (!existingPOBuffer || existingPOBuffer.toString().length <= 0) {
      ...
      let sellerCompanyBuffer = await ctx.stub.getState(
        existingPOObject.seller
      );
      ...
      helper.validateUserRole(
        ctx,
        [sellerCompanyObject.organisationRole.toLowerCase()],
        "Shipment can be created onlyby the seller"
      );
      ...
      if (drugSerialNoArr.length < existingPOObject.quantity) {
        throw new Error(
          "Shipment creation error: List of assets insuficient for creating shipment"
        );
      }
      ...
      for (let i = 0; i < drugSerialNoArr.length; i++) {
        ...
        if (
          !(
            !existingDrugBuffer ||
            existingDrugBuffer.toString().length <= 0 ||
            existingDrugObject.owner !== existingDrugObject.seller
          )
        ) {
          throw new Error(
            "Invalid drug key: " +
              ...
              "+" +
              drugSerialNoArr[i] +
              ". Drug does not exist or is not owned by the seller."
          );
        } else {
      ...
      let transporterCompanyObject = await helper.getCompanyByCRN(
        ctx,
        transporterCRN
      );
  ...
  async updateShipment(ctx, buyerCRN, drugName, transporterCRN) {
    ...
    helper.validateUserRole(
      ctx,
      ["transporter"],
      "shipment can be delivered by a Transporter only."
    );
    ...
    if (
      !existingShipmentbuffer ||
      existingShipmentbuffer.toString().length <= 0
    ) {
      throw new Error("Shipment details not found");
    } else {
      // convert to shipment object for updates
      let existingShipmentObject = JSON.parse(
        existingShipmentbuffer.toString()
      );
      ...
      if (
        transporterCompanyObject.companyID !==
        existingShipmentObject.transporter
      ) {
        throw new Error(
          "Only the transporter of the shipment can mark the shipment as delivered"
        );
      }
      ...
      for (let i = 0; i < existingShipmentObject.assets.length; i++) {
        ...
        let existingDrugBuffer = await ctx.stub.getState(
          existingShipmentObject.assets[i]
        );
      ...
      let newShipmentBuffer = Buffer.from(
        JSON.stringify(existingShipmentObject)
      );
  ...
  async retailDrug(ctx, drugName, serialNo, retailerCRN, customerAadhar) {
    ...
    helper.validateUserRole(
      ctx,
      ["retailer"],
      "only retailer would be able to sell the drug to consumer."
    );
    ...
    if (retailerCompanyObject.companyID !== existingDrugObject.owner) {
      throw new error(
        "Retailer is not the owner fo the Drug : " + drugName + ":" + serialNo
      );
    } else {
    ...
    if (!existingDrugBuffer || existingDrugBuffer.toString().length <= 0) {
      throw new Error(
        "Invalid drug key: " +
          ...
          "+" +
          serialNo +
          ". A drug with this ID does not exist."
      );
    } else {
