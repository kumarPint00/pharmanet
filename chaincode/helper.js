...
class helper {
  ...
  validateUserRole(ctx, role, errMsg) {
    ...
    if (isValidUser === false) {
      throw new Error(
        "User Authorization Failed. Initiator Role: " +
          initiatorID.issuer.organizationName.split(".")[0] +
          " Error MSG: " +
          errMsg
      );
    }
  ...
  async getCompanyByCRN(ctx, companyCRN) {
    ...
    let companyResultsIterator = await ctx.stub.getStateByPartialCompositeKey(
      companyNameSpace,
      [companyCRN]
    );
